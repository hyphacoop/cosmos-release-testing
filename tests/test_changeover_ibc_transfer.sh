...
consumer_expected_denom=ibc/$(echo -n transfer/channel-0/uatom | shasum -a 256 | cut -d ' ' -f1 | tr '[a-z]' '[A-Z]')
provider_expected_denom=ibc/$(echo -n transfer/channel-0/ucon | shasum -a 256 | cut -d ' ' -f1 | tr '[a-z]' '[A-Z]')
...
command="$CONSUMER_CHAIN_BINARY --home $CONSUMER_HOME_1 tx ibc-transfer transfer transfer channel-0 $WALLET_1 1000$CONSUMER_DENOM --from $RECIPIENT --keyring-backend test --gas auto --gas-adjustment $GAS_ADJUSTMENT --gas-prices $GAS_PRICE$CONSUMER_DENOM -y -o json"
