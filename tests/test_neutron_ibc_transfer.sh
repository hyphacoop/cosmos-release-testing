...
command="$CONSUMER_CHAIN_BINARY --node http://localhost:$CON1_RPC_PORT --home $CONSUMER_HOME_1 --chain-id $CONSUMER_CHAIN_ID tx ibc-transfer transfer transfer channel-1 $WALLET_1 1000$CONSUMER_DENOM --from $RECIPIENT --keyring-backend test --gas auto --gas-adjustment $GAS_ADJUSTMENT --gas-prices $GAS_PRICE$CONSUMER_DENOM -y -o json"
