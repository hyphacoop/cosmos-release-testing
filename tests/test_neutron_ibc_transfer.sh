...
$CONSUMER_CHAIN_BINARY --node http://localhost:$CON1_RPC_PORT q bank balances $RECIPIENT
consumer_start_balance=$($CONSUMER_CHAIN_BINARY --node http://localhost:$CON1_RPC_PORT q bank balances $RECIPIENT -o json | jq -r --arg DENOM "$consumer_expected_denom" '.balances[] | select(.denom==$DENOM).amount')
...
command="$CHAIN_BINARY --node http://localhost:$CON1_RPC_PORT tx ibc-transfer transfer transfer $PROVIDER_CHANNEL $RECIPIENT 1000$DENOM --from $WALLET_1 --keyring-backend test --gas $GAS --gas-adjustment $GAS_ADJUSTMENT --gas-prices $GAS_PRICE$DENOM -y"
echo $command
$command
...
consumer_end_balance=$($CONSUMER_CHAIN_BINARY --node http://localhost:$CON1_RPC_PORT q bank balances $RECIPIENT -o json | jq -r --arg DENOM "$consumer_expected_denom" '.balances[] | select(.denom==$DENOM).amount')
...
command="$CONSUMER_CHAIN_BINARY --node http://localhost:$CON1_RPC_PORT tx ibc-transfer transfer transfer channel-1 $WALLET_1 1000$CONSUMER_DENOM --from $RECIPIENT --keyring-backend test --gas auto --gas-adjustment $GAS_ADJUSTMENT --gas-prices $GAS_PRICE$CONSUMER_DENOM -y -o json"
txhash=$($command | jq -r .txhash)
echo "Waiting for the transfer to reach the provider chain..."
sleep $(($COMMIT_TIMEOUT*20))
echo "tx hash: $txhash"
$CONSUMER_CHAIN_BINARY q tx $txhash --node http://localhost:$CON1_RPC_PORT
