...
txhash_1=$($CHAIN_BINARY tx gov submit-proposal proposal.json --from $WALLET_1 --gas $gas --gas-prices $GAS_PRICE --home $HOME_1 -y -o json | jq -r '.txhash')
txhash_2=$($CHAIN_BINARY tx gov submit-proposal proposal.json --from $WALLET_2 --gas $gas --gas-prices $GAS_PRICE --home $HOME_1 -y -o json | jq -r '.txhash')
