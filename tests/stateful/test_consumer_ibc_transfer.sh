...
$CHAIN_BINARY --home $HOME_1 tx ibc-transfer transfer transfer $PROVIDER_CHANNEL $CONSUMER_WALLET $AMOUNT$DENOM --from $PROVIDER_WALLET --keyring-backend test --gas 500000 --fees 5000$DENOM -b sync -y --chain-id $CHAIN_ID
...
$CONSUMER_CHAIN_BINARY --home $CONSUMER_HOME_1 tx ibc-transfer transfer transfer channel-1 $PROVIDER_WALLET $AMOUNT$CONSUMER_DENOM --from $CONSUMER_WALLET --keyring-backend test -y -b sync --gas 500000 --fees 5000$CONSUMER_DENOM
