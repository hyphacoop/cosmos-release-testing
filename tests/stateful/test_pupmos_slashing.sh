...
submit_proposal_cmd="gaiad --home $HOME_1 tx provider submit-consumer-double-voting tests/stateful/double-signed-evidence.json tests/stateful/double-signed-ibc-header.json --from $MONIKER_1 --gas $GAS --gas-adjustment $GAS_ADJUSTMENT --gas-prices $GAS_PRICES$DENOM --chain-id $CHAIN_ID -y -b sync -o json"
