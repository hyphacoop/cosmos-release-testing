...
$CHAIN_BINARY tx vesting create-vesting-account $vesting_wallet1_addr $vesting_amount$DENOM $vesting_end_time --from validator --gas $GAS --gas-adjustment $GAS_ADJUSTMENT --fees $BASE_FEES$DENOM
$CHAIN_BINARY q bank spendable-balances $vesting_wallet1_addr -o json | jq -r '.balances[] | select(.denom="uatom") | .amount'
...
count=0
current_spend_amount=$($CHAIN_BINARY q bank spendable-balances $vesting_wallet1_addr -o json | jq -r '.balances[] | select(.denom="uatom") | .amount')
...
timestamp=$($CHAIN_BINARY q block --type=height $current_height -o json | jq -r '.header.time')
echo $timestamp
