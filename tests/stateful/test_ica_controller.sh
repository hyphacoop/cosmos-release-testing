...
connection_id=$(hermes  --json query client connections --client 07-tendermint-70 --chain $CHAIN_ID | jq -r '.result[0]')

echo "Registering ICA..."
$CHAIN_BINARY tx interchain-accounts controller register connection-0 --from $WALLET_1 --gas auto --gas-adjustment $GAS_ADJUSTMENT --gas-prices $GAS_PRICE$DENOM -y -o json --home $CHAIN_HOME
sleep 60

ica_address=$($CHAIN_BINARY q interchain-accounts controller interchain-account $WALLET_1 connection-0 --home $CHAIN_HOME -o json | jq -r '.address')
...
$CHAIN_BINARY_SECONDARY q bank balances $WALLET_5 --home $HOST_HOME -o json | jq '.'
...
echo "Sending ICA tx..."
$CHAIN_BINARY tx interchain-accounts controller send-tx connection-0 send_packet.json --from $WALLET_1 --gas auto --gas-adjustment $GAS_ADJUSTMENT --gas-prices $GAS_PRICE$DENOM -y -o json --home $CHAIN_HOME
sleep 60
...
recipient_balance=$($CHAIN_BINARY_SECONDARY q bank balances $WALLET_5 --home $HOST_HOME -o json | jq -r '.balances[] | select(.denom == "uatom").amount')
