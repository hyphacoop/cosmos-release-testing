...
echo "> Opting in"
txhash=$($CHAIN_BINARY tx provider opt-in $consumer_id --from ${monikers[-1]} --gas $GAS --gas-adjustment $GAS_ADJUSTMENT --gas-prices $GAS_PRICE --home $whale_home -y -o json | jq -r .txhash)
sleep $(($COMMIT_TIMEOUT*5))
echo "Txhash: $txhash"
echo "> Querying opt-in transaction."
$CHAIN_BINARY --output json q tx $txhash --home $whale_home | jq '.'
