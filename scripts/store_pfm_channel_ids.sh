...
$CHAIN_BINARY q ibc connection connections -o json --home $whale_home | jq -r --arg client "$pfm_ba_client_id" '.connections[] | select((.client_id==$client) and (.state=="STATE_OPEN")).id'
pfm_ba_connection_id=$($CHAIN_BINARY q ibc connection connections -o json --home $whale_home | jq -r --arg client "$pfm_ba_client_id" '.connections[] | select((.client_id==$client) and (.state=="STATE_OPEN")).id')
