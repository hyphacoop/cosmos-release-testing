...
while [ $count -lt $1 ]
do
current_height=$(curl -s 127.0.0.1:26660/metrics | \
grep "cometbft_consensus_height{chain_id=\"provider\"}" | \
awk -F " " '{ print $2 }' | awk -F"E" 'BEGIN{OFMT="%10.10f"} {print $1 * (10 ^ $2)}')

current_size=$(curl -s 127.0.0.1:26660/metrics | \
grep "cometbft_consensus_block_size_bytes{chain_id=\"provider\"}" | \
