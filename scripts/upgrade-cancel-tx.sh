...
echo "> Submitting the cancel proposal tx"
cancel_proposal="$CHAIN_BINARY --output json tx gov cancel-proposal $proposal_id --from $WALLET_1 --keyring-backend test --chain-id $CHAIN_ID --gas $GAS --gas-prices $GAS_PRICE --gas-adjustment $GAS_ADJUSTMENT -y --home $whale_home -o json"
