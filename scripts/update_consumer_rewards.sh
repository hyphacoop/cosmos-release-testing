...
echo "> Save hash to GH environment variable"
echo "> Tx hash: $txhash"
echo "REWARD_TX_HASH=$txhash" >> $GITHUB_ENV
echo "> Save block height to GH environment variable"
height=$($CHAIN_BINARY q tx $txhash --home $whale_home -o json | jq -r .height)
echo "> Height: $height"
echo "REWARD_TX_HEIGHT=$height" >> $GITHUB_ENV
