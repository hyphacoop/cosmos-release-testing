...
vote="$CHAIN_BINARY tx gov vote $proposal_id yes --from $WALLET_2 --home $HOME_1 --gas $GAS --gas-adjustment $GAS_ADJUSTMENT --gas-prices $GAS_PRICE$DENOM -y -o json"
vote="$CHAIN_BINARY tx gov vote $proposal_id yes --from $WALLET_3 --home $HOME_1 --gas $GAS --gas-adjustment $GAS_ADJUSTMENT --gas-prices $GAS_PRICE$DENOM -y -o json"
