...
submit_tx "tx bank send $WALLET_1 $happy_liquid_3 $funding$DENOM --from $WALLET_1 --gas auto --gas-adjustment $GAS_ADJUSTMENT --gas-prices $GAS_PRICE -o json -y" $CHAIN_BINARY $whale_home
submit_tx "tx bank send $WALLET_1 $happy_owner    $funding$DENOM --from $WALLET_1 --gas auto --gas-adjustment $GAS_ADJUSTMENT --gas-prices $GAS_PRICE -o json -y" $CHAIN_BINARY $whale_home
